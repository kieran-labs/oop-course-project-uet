...
public class AutoBidConfigDao {
  ...
  private static class AutoBidConfigMapper implements RowMapper<AutoBidConfig> {
    ...
    public AutoBidConfig map(ResultSet rs, StatementContext ctx) throws SQLException {
      var registeredAt = rs.getTimestamp("registered_at").toLocalDateTime();

      // [FIX #11] Truyền registeredAt cho cả hai: registeredAt VÀ createdAt.
      // Lý do: bảng auto_bid_configs chỉ có registered_at, không có created_at.
      // Recommend: thêm method getCreatedAt() trong AutoBidConfig override
      // trả về registeredAt → loại bỏ hoàn toàn tham số createdAt ở đây.
