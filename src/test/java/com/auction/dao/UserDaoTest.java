...
/**
 * Test cho UserDao - Đảm bảo các nghiệp vụ CRUD và tính đa hình. Liên kết: model/User.java,
...
class UserDaoTest {
  ...
  /**
   * Dọn dẹp bảng users trước mỗi test case. Sử dụng Transaction để đảm bảo tính toàn vẹn và tránh
   * lỗi Permission.
   */
  @BeforeEach
  void cleanup() {
    jdbi.useTransaction(
        handle -> {
          ...
          handle
              .createUpdate(
                  "DELETE FROM users WHERE username LIKE 'test_%' OR username LIKE 'poly_%' OR username = 'duplicate_user'")
              .execute();
        });
  }
