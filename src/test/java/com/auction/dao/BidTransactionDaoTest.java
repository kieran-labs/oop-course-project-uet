...
class BidTransactionDaoTest {
  ...
  static void setup() {
    ...
    testSeller =
        userDao.insert(
            new Seller(
                "bid_tx_seller_" + timestamp, "hash", "bid_tx_seller_" + timestamp + "@test.com"));

    testBidder =
        userDao.insert(
            new Bidder(
                "bid_tx_bidder_" + timestamp, "hash", "bid_tx_bidder_" + timestamp + "@test.com"));

    testItem =
        itemDao.insert(
            new Electronics("Bid Tx Item " + timestamp, "Test", testSeller.getId(), "Brand"));
  ...
  void testFindByAuctionId() {
    ...
    assertTrue(
        bids.get(0).getCreatedAt().isBefore(bids.get(1).getCreatedAt())
