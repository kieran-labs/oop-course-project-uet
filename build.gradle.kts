...
dependencies {
    ...
    // ── DATABASE RUNTIME ────────────────────────────────────────────────────
    // Embedded PostgreSQL: chế độ mặc định khi KHÔNG set DB_URL.
    // Khi DB_URL được set, DatabaseConfig sẽ bỏ qua embedded startup và kết nối
    // đến PostgreSQL ngoài bằng DB_URL, DB_USER, DB_PASSWORD.
    // Trong cả 2 chế độ, stack database vẫn là:
    //   PostgreSQL JDBC → HikariCP → Flyway → JDBI.
    // Người chấm chạy JAR bình thường không cần cài PostgreSQL riêng.
    // Runtime embedded lưu dữ liệu bền tại data/postgres.
    // Lần đầu chạy embedded có thể tải/cache binary PostgreSQL phù hợp với OS.
...
// ============================================================================
// Hai JAR được tạo ra:
//   auction-server.jar → chạy Javalin server + PostgreSQL runtime
//                        (Embedded mặc định hoặc external qua DB_URL)
...
// Embedded PostgreSQL tự khởi động khi không có DB_URL; nếu có DB_URL,
// server kết nối PostgreSQL ngoài bằng DB_URL, DB_USER, DB_PASSWORD.
// Embedded mode lưu dữ liệu tại data/postgres.
//
// Cách dùng:
